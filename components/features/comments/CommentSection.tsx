...
import { listComments, type CommentNode } from '@/lib/db/queries/comments';
...
export async function CommentSection({ targetType, targetId, loginNext }: Props) {
...
function CommentItem({
  ...
  const displayName = node.is_anonymous ? '익명' : (node.author?.nickname ?? '주민');
    ...
    <div className={depth > 0 ? 'ml-6 border-l-2 border-border pl-4' : ''}>
      <article className="rounded-md border border-border bg-white p-3">
        <header className="flex items-center justify-between">
          <span className="text-sm font-medium text-brand-primary">{displayName}</span>
